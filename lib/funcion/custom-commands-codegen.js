...
import { eliminarComando as eliminarPermisosComando } from './custom-command-group-permissions.js';
...
function armarCodigoFuente(commandName, sesion) {
  ...
  code += 'import { puedeUsarComando } from \'../lib/funcion/custom-command-group-permissions.js\'\n\n';
  code += `const handler = async (m, { conn, participants }) => {
  if (m.chat.endsWith('@g.us') && !puedeUsarComando('${commandName}', m.chat)) return
...
function eliminarComando(commandName) {
  ...
  eliminarPermisosComando(commandName);
}
