...
export async function handleParticipantsUpdate(
  ...
  try {
    ...
    const eventTimestamp =
      (mconn?.messageTimestamp ||
       mconn?.key?.messageTimestamp ||
       mconn?.timestamp || 0) * 1000;

    if (eventTimestamp && eventTimestamp < connectionTime - 60000) {
      return;
    }
    ...
    let authorJid = mconn?.key?.participant || mconn?.participant;

    if (!authorJid) {
      authorJid = mconn?.conn?.user?.jid;
    }

    const deduplicateKey = `${id}_${normalizedAction}_${Date.now()}`;
    ...
    if (normalizedAction === 'add' || normalizedAction === 'remove') {
      ...
      if (actionDeduplicator.has(deduplicateKey)) {
        return;
      }
      actionDeduplicator.set(deduplicateKey, true);
      
      setTimeout(() => actionDeduplicator.delete(deduplicateKey), 1000);
