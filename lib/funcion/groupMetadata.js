...
export async function sendWelcomeOrGoodbye(conn, id, messageData) {
  try {
    ...
    const { text, apii, userJid, customImage } = messageData;
  ...
  try {
    ...
    if (normalizedAction === 'add' || normalizedAction === 'remove') {
      ...
      for (const participant of participantsList) {
        ...
        if (normalizedAction === 'add') addParticipant(id, userJid, participant.lid || null);
        else removeParticipant(id, userJid);
