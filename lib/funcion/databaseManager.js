import { createUsersProxy, migrateFromLowdb, setUser, USER_DEFAULTS } from './userManager.js';
...
export async function ensureUserData(sender, chat) {
  try {
    ...
    Object.keys(USER_DEFAULTS).forEach(key => {
      if (user[key] === undefined) user[key] = USER_DEFAULTS[key];
    });
