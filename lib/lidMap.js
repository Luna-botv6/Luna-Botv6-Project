...
function ensureDir() {
  try {
    ...
    console.error('[lidMap] Error creando directorio:', e.message);
  }
}
...
export async function saveLidMap() {
...
export function registerLid(lid, jid) {
  ...
  saveLidMap().catch(e => console.error('[lidMap] Error guardando:', e.message));
  return true;
}
