...
async function purgeSessionSB() {
  if (!fs.existsSync('./jadibts/')) {
    console.log(chalk.cyanBright('📁 [cleaner] La carpeta jadibts no existe, se omite la limpieza.'));
    return;
  }

  console.log(chalk.cyanBright('✨ [cleaner] Iniciando limpieza en jadibts...'));
  ...
  for (const directorio of listaDirectorios) {
    ...
    if (stats.isDirectory()) {
      await deleteFilesInBatches(dirPath, file =>
        file.startsWith('pre-key-') ||
        file.startsWith('sender-key-') ||
        file.startsWith('app-state-sync-key-'));
    }
...
async function main() {
  console.log(chalk.cyanBright('🚀 [cleaner] Iniciando limpieza general...'));
  await purgeSession();
  await purgeOldFiles();
