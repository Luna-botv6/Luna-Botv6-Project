...
function extraerLinks(texto) {
  if (typeof texto !== 'string' || !texto.length) return [];
  const out = [];
...
function setUrlCached(url, flagged) {
  _urlCache.set(url, { flagged, ts: Date.now() });
  if (_urlCache.size > MAX_URL_CACHE) {
    const oldestKey = _urlCache.keys().next().value;
    _urlCache.delete(oldestKey);
  }
}

async function esLinkContenido18(texto) {
  const urls = extraerLinks(texto);
  for (const url of urls) {
    ...
    try {
      const res = await fetchWithTimeout(SERVER_URL + '/classify-url', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': API_KEY },
      ...
      if (!success) {
        console.error('[anti18] server de IA no clasificó el link, se deja pasar');
      }
    } catch (e) {
      console.error('[anti18] error consultando server de IA por link:', e.message);
    } finally {
      releaseSlot();
    }

    if (success) setUrlCached(url, flagged);
    if (flagged) return true;
  }
  return false;
}

async function bloquearContenido({ m, conn, grupoData: groupData, isOwnerSender, isPrivilegiado, warnReason, t, msgEliminado }) {
  const tag = m.sender.split('@')[0];

  if (isPrivilegiado) {
    const frase = isOwnerSender ? elegirFrase(t.frases_owner || FRASES_OWNER) : elegirFrase(t.frases_admin || FRASES_ADMIN);
  ...
  try {
    ...
    console.error('[anti18] error borrando mensaje:', e.message);
  }

  const warns = await addWarning(m.sender, warnReason, 'AntiNSFW', m.sender);

  await conn.sendMessage(m.chat, {
    text: (msgEliminado || t.contenido_eliminado || '🔞 *Contenido +18 detectado*\n\n@{tag} tu mensaje fue eliminado, este grupo no permite contenido para adultos.\n📌 Advertencia {warns}/{max}')
      .replace('{tag}', tag).replace('{warns}', warns).replace('{max}', MAX_WARNS),
    mentions: [m.sender]
  });
...
handler.before = async function (m, { conn }) {
  ...
  if (tipo === 'imageMessage' || tipo === 'stickerMessage') {
    try {
      ...
      await bloquearContenido({
        m, conn, grupoData: groupData,
        isOwnerSender,
        isPrivilegiado,
        warnReason: 'Contenido +18 detectado automáticamente',
        t
      });
    } catch (e) {
  ...
  if (typeof m.text === 'string' && m.text.length && extraerLinks(m.text).length) {
    try {
      ...
      if (!flagged) return;
      return await bloquearContenido({
        m, conn, grupoData: groupData,
        isOwnerSender,
        isPrivilegiado,
        warnReason: 'Enlace con contenido +18 detectado automáticamente',
        t,
        msgEliminado: t.link_eliminado || '🔞 *Enlace +18 detectado*\n\n@{tag} el link que enviaste fue eliminado, este grupo no permite contenido para adultos.\n📌 Advertencia {warns}/{max}'
      });
    } catch (e) {
      const detalle = e instanceof Error ? (e.stack || e.message) : (JSON.stringify(e) ?? String(e));
      console.error('[anti18] error procesando link:', detalle);
    }
  }
};
