...
const handler = async (m, { isOwner, conn, text, usedPrefix }) => {
  ...
  if (!isAdmin && !isOwner) return m.reply(t_handler.texto7);
  ...
  if (handler._cooldowns.has(chatId)) {
    ...
    if (now < exp) {
      ...
      return m.reply(`${tradutor.texto1} ${Math.floor(left / 60)}m ${left % 60}s`);
    }
  }
  ...
  if (fantasmas.length === 0) return m.reply(tradutor.texto2);

  const groupName = await conn.getName(m.chat);

  const lista = fantasmas
    .map((f, i) => `> ${i + 1}. @${f.numero}  ➜  ${tradutor.texto3}  ✦  ${f.count} msg`)
    .join('\n');
