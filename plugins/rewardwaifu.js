...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  const t = _tr?.plugins?.rewardwaifu || {};
  let user = getUserData(m.sender);
  let now = Date.now();
  let cooldown = 24*60*60*1000;
  if (!user.harem || user.harem.length===0) return await conn.reply(m.chat, t.sinWaifus?.replace('{prefix}', usedPrefix) || `❌ No tienes waifus reclamada. Usa ${usedPrefix}rw y ${usedPrefix}claim`, m);
  if (user.lastReward && now-user.lastReward<cooldown){
    ...
    return await conn.reply(m.chat, t.cooldown?.replace('{horas}', hrs).replace('{minutos}', mins) || `⏳ Ya recibiste tu recompensa hoy.\nVuelve en ${hrs}h ${mins}m.`, m);
  }
  ...
  if(xpReward===0&&moneyReward===0) return await conn.reply(m.chat,t.sinValor || '💤 Tus waifus aún no tienen suficiente valor para recompensa.',m);
  addExp(m.sender,xpReward);
  addMoney(m.sender,moneyReward);
  user.lastReward=now;
  setUserData(m.sender,user);
  await conn.reply(m.chat,t.recompensa?.replace('{valor}', totalValor).replace('{xp}', xpReward).replace('{monedas}', moneyReward) || `🎁 Recompensa Gacha 🎁\n\nValor total del harem: ${totalValor}\nXP ganada: +${xpReward}\nMonedas ganadas: +${moneyReward}\n\nSigue coleccionando waifus para obtener más recompensas.`,m);
};
