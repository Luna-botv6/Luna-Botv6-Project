...
const handler = async (m, { conn, isOwner }) => {
  ...
  const t = _translate.plugins.gc_imgbye;
  ...
  const imgPath = `${WELCOME_DIR}/${chatClean}_bye.jpg`;

  const isDelete = /^(delbyeimg|eliminarbyeimg)$/i.test(m.text?.trim() || '');
...
handler.help = ['imgbye'];
handler.tags = ['group'];
handler.command = /^(imgbye|delbyeimg)$/i;
handler.group = true;
