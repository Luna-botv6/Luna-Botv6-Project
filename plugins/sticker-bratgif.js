...
let handler = async (m, { conn, text }) => {
    ...
    const t = _tr?.plugins?.sticker_bratgif || {};
    if (m.quoted?.text) {
        text = m.quoted.text
    }
    ...
    try {
        ...
        const pack = user.text1 || global.packsticker || global.botname
        const author = user.text2 || global.packsticker2 || global.botname

        const res = await ft(
            SERVER_URL + '/api/sticker/bratgif?text=' + encodeURIComponent(text) +
