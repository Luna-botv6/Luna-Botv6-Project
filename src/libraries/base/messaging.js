  ...
  async sendFile(conn, jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
    ...
    if (/webp/.test(type.mime) || (/image/.test(type.mime) && options.asSticker)) {
      ...
      try {
        ...
        console.error('Error convirtiendo audio:', e.message);
        mtype = 'document';
      }
    } else {
      mtype = 'document';
...
N:;${name.replace(/\n/g, '\\n')};;;
FN:${name.replace(/\n/g, '\\n')}
TEL;type=CELL;type=VOICE;waid=${number}:${number}${biz.description ? `
...
END:VCARD`.trim();
        ...
        console.error(`Error obteniendo perfil de ${number}:`, e.message);
        contacts.push({
